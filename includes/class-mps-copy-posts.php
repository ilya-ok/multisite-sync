...
/**
 * Копирование записей (post) с главного сайта на дочерние.
 * Запись идентифицируется по post_name (slug).
 * Копируются: заголовок, slug, контент, excerpt, статус,
 * кастомные мета-поля, категории (создаются на дочернем если нет).
 */
class MPS_Copy_Posts {

	private static $instance = null;
	private $page_hook       = '';

	/** Кастомные мета-поля, которые нужно переносить */
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_mps_get_posts',  array( $this, 'ajax_get_posts' ) );
		add_action( 'wp_ajax_mps_copy_post',  array( $this, 'ajax_copy_post' ) );
	}
	...
	public function add_submenu_page() {
		$this->page_hook = add_submenu_page(
			'ms-bulk-edit',
			__( 'Копирование записей', 'multisite-sync' ),
			__( 'Копирование записей', 'multisite-sync' ),
			'manage_network',
			'mps-copy-posts',
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script(
			'mps-copy-posts',
			MS_PLUGIN_URL . 'assets/js/copy-posts.js',
			array( 'jquery' ),
			MS_VERSION,
			true
		);
		...
		wp_localize_script( 'mps-copy-posts', 'mpsCopyPostsData', array(
			'ajaxurl' => admin_url( 'admin-ajax.php' ),
			'nonce'   => wp_create_nonce( 'mps_copy_posts_nonce' ),
			'sites'   => $sites_data,
		) );
	}
	...
	public function render_page() {
		...
		?>
		<div class="wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'Копирование записей', 'multisite-sync' ); ?></h1>

			<p class="description">
				<?php esc_html_e( 'Записи (посты) главного сайта. Нажмите «Копировать» — запись будет создана или обновлена на всех дочерних сайтах вместе с категориями и мета-полями.', 'multisite-sync' ); ?>
			</p>

			<div class="ms-section">
				<div class="ms-bulk-header">
					<button id="mps-load-posts" class="button button-primary">
						<?php esc_html_e( 'Загрузить записи', 'multisite-sync' ); ?>
					</button>
					<span id="mps-post-count" class="ms-count"></span>
			...
			<div id="mps-posts-container" style="display:none;"></div>
	...
	public function ajax_get_posts() {
		check_ajax_referer( 'mps_copy_posts_nonce', 'nonce' );
		...
		// Проверяем наличие записей на дочерних сайтах
		...
		switch_to_blog( $main_id );
		$html = $this->build_posts_table( $posts, $child_sites, $site_status );
		restore_current_blog();
	...
	private function build_posts_table( array $posts, array $child_sites, array $site_status ): string {
		...
		$html .= '<th class="ms-col-name">'    . esc_html__( 'Запись', 'multisite-sync' ) . '</th>';
		...
		foreach ( $posts as $post ) {
			...
			$html .= '<button class="mps-copy-post-btn button button-primary" data-post-id="' . esc_attr( $post->ID ) . '">';
			...
			if ( ! empty( $cat_names ) ) {
				$html .= '<br><span class="ms-post-cats">' . esc_html( implode( ', ', $cat_names ) ) . '</span>';
			}
	...
	public function ajax_copy_post() {
		...
		// Категории: сохраняем slug и name для воссоздания на дочернем
		$source_cats = array();
		...
		foreach ( $all_sites as $site ) {
			...
			// Находим или создаём категории
			$cat_ids = array();
			foreach ( $data['cats'] as $cat_data ) {
				$term = get_term_by( 'slug', $cat_data['slug'], 'category' );
				if ( $term ) {
					$cat_ids[] = $term->term_id;
				} else {
					$new_term = wp_insert_term( $cat_data['name'], 'category', array( 'slug' => $cat_data['slug'] ) );
			...
			// Ищем существующую запись по slug
			global $wpdb;
