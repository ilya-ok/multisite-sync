/**
 * Копирование страниц между сайтами мультисайта
 */
...
(function($) {
	...
	function loadPages() {
		$('.mps-loading').show();
		$('#mps-pages-container').hide().empty();
		$('#mps-page-count').text('');

		$.ajax({
			url:  mpsCopyPagesData.ajaxurl,
			type: 'POST',
			data: {
				action: 'mps_get_pages',
				nonce:  mpsCopyPagesData.nonce
			},
			success: function(response) {
				$('.mps-loading').hide();

				if (response.success) {
					$('#mps-pages-container').html(response.data.html).show();
					if (response.data.count > 0) {
						$('#mps-page-count').text('(' + response.data.count + ')');
					}
				} else {
	...
	function copyPage() {
		...
		var sites   = mpsCopyPagesData.sites;
	...
	$(document).ready(function() {
		$('#mps-load-pages').on('click', loadPages);
		$(document).on('click', '.mps-copy-page-btn', copyPage);
	});
